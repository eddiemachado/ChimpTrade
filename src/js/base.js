...
function updatePrices() {
  ...
  /*
  // pull the alert prices and start running checks
  chrome.storage.sync.get('alertPrices', function(data) {
    for (var i = 0; i < data.alertPrices.length; i++) {
      var obj = data.alertPrices[i];
      var coin = obj.alertCoin;
      var currentPrice = obj.alertCurrentPrice;
      // look for the alerts with the same name & update the prices
      $('.alert-current-' + coin).text(numeral(currentPrice).format('$0,0[.]00'));
    }
  });
  */
}
    ...
    fetch(priceAPI).then(function(res) {
      ...
      res.json().then(function(data) {
        ...
        if (coinName) {
          ...
          $('#alert-current-price').val(data[coinName].usd);
          validator.removeClass('coin-error');
          validator.addClass('coin-found');
          feedback.removeClass('is-wrong');
        }
        // if there's no match
        else {
...
// convert currencies (btc <-> eth <-> bnb <-> usd)
...
function calculateTrade() {
  ...
  chrome.storage.local.get('currency', function(data) {
    ...
    switch (currency) {
      ...
      case 'bnb':
        // format the totals
        output = numeral(totalReturn).format('0.000000');
        total.text(numeral(output).format('0.000000') + ' BNB');
        profit.text(numeral(totalProfit).format('0.000000') + ' BNB');
        break;
      default:
        // format the totals
